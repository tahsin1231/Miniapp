...
━━━━━━━━━━━━━━━━━━━━━`,
      ...
      const unsub = onSnapshot(userRef, async (docSnap) => {
        if (docSnap.exists()) {
          ...
          const initialData: UserData = {
            telegramId: tgUser?.id.toString() || 'unknown',
          ...
          setDoc(userRef, initialData).then(() => {
            if (tgUser && currentSettingsLocal) {
              sendNewUserAlert(tgUser, currentSettingsLocal);
            }
  ...
  const handleWatchAd = async () => {
    ...
    try {
      // Open Monetag Ad Link
      let adUrl = currentSettings.monetagZoneId || '10754815';
      
      // If it's just an ID, construct the standard Monetag link
      if (!adUrl.startsWith('http')) {
        adUrl = `https://alwingulla.com/${adUrl}`;
      }
      ...
      // Wait for a few seconds to simulate "watching" time before crediting
      // This ensures the user actually sees the ad window opening
      await new Promise(resolve => setTimeout(resolve, isNextUnique ? 12000 : 7000));
      
      const userRef = doc(db, 'users', user.uid);
      
      if (isNextUnique) {
        // Unique Ad Logic: User must click and register
        await updateDoc(userRef, {
          earnings: increment(reward),
          adsWatched: increment(1),
          lastAdWatchedAt: new Date().toISOString(),
          currentAdCount: 0 // Reset counter
        });
        alert('Unique Ad reward credited! Thank you for registering.');
      } else {
        // Common Ad Logic
        await updateDoc(userRef, {
          earnings: increment(reward),
                ...
                <div className="relative z-10">
                  ...
                  <div className="mt-8 flex items-center gap-6">
                    <div className="flex flex-col">
                      ...
                      <span className="text-[10px] font-bold text-blue-200/60 uppercase tracking-widest">Ads Watched</span>
                    </div>
              ...
              <div className="space-y-4">
                ...
                <div className="bg-slate-900/50 border border-slate-800 p-4 rounded-3xl flex items-center gap-3">
                  <input 
                    type="text" 
                    readOnly 
                    value={`https://t.me/${currentSettings.botUsername}?start=${userData?.telegramId}`}
